...
- Authenticated (optional, not default): requires `VERIFY_CREEK_ALLOW_AUTH=1` and an explicit non-production token. Skip with unmet-precondition rather than reading `~/.creek`.
...
If `CREEK_TOKEN` is already set in the agent environment and `VERIFY_CREEK_ALLOW_AUTH` is not `1`, the script records `unmet-precondition` and exits 2 (does not send that token to api.creek.dev). Otherwise it runs `whoami --json` under isolated HOME and asserts the unauthenticated JSON.

...
### Run the D1 migrations

The schema is managed by Drizzle migrations under `packages/control-plane/drizzle`
(wired up via `migrations_dir` in `wrangler.toml`). Apply them to your remote D1:

```bash
cd packages/control-plane
npx wrangler d1 migrations apply creek-db --remote
```

This tracks which migrations have run, so it is safe to re-run — only new ones
are applied.
...
**D1 schema errors on deploy** — re-run `wrangler d1 migrations apply creek-db --remote`. It only applies migrations that haven't run yet, so it's safe to repeat.
...
Pull new commits and re-run the deploy commands. Apply any new DB migrations with `wrangler d1 migrations apply creek-db --remote` — already-applied migrations are skipped, so it's safe to repeat.

...
| Build log (`get_build_log`) | API key argument |
| Resources (`list_resources`, `create_resource`, `attach_resource`, `detach_resource`, `delete_resource`, `rename_resource`, `query_database`) | API key argument |
...
Prefer the CLI when you have a shell — authenticated MCP tools currently take `apiKey` as a parameter, which puts the key in the transcript. The CLI reads `~/.creek/config.json` or `CREEK_TOKEN`.

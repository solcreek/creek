...
app.use("*", async (c, next) => {
  const allowed = (c.env.ALLOWED_ORIGINS || "https://creek.dev").split(",");
  return cors({
    origin: allowed,
...
// Health
app.get("/", (c) => c.json({ status: "ok", service: "creek-deploy-api" }));

// POST /deploy — trigger build + deploy
  ...
  if (body.type === "template") {
    ...
    try {
      ...
      if (!["github.com", "gitlab.com", "bitbucket.org"].includes(url.hostname)) {
        return c.json({ error: "Only GitHub, GitLab, and Bitbucket repos are supported" }, 400);
      }
    } catch {
...
app.get("/deploy/:buildId/status", async (c) => {

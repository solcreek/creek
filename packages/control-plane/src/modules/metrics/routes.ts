...
import { totalsSql, timeseriesSql, breakdownSql, type BreakdownDimension } from "./queries.js";
...
metrics.get("/:slug/metrics", requirePermission("project:read"), async (c) => {
  ...
  const customDomainRows = await c.env.DB.prepare(
    ...
    .bind(project.id)
    .all<{ hostname: string }>();
  const customHostnames = (customDomainRows.results ?? []).map((r) => r.hostname);

  if (!c.env.CLOUDFLARE_API_TOKEN || !c.env.CLOUDFLARE_ACCOUNT_ID) {
    return c.json({ error: "metrics_unavailable", message: "AE credentials missing" }, 503);
  }
  ...
  const dimensions: BreakdownDimension[] = ["method", "scriptType", "statusBucket"];
  ...
  try {
    const [totals, series, zone, ...breakdowns] = await Promise.all([
      querySql<{ reqs: number | null; errs: number | null }>(c.env, totalsSql(scope)),
      querySql<{ bucket: number; reqs: number; errs: number }>(c.env, timeseriesSql(scope)),
      queryZoneHttpAnalyticsMerged(c.env, allHostnames, periodHours, c.env.CREEK_DOMAIN),
      ...dimensions.map((d) =>
        querySql<{ label: string; reqs: number; errs: number }>(c.env, breakdownSql(scope, d)),
      ),
    ]);
    ...
    return c.json({ error: "metrics_query_failed", message: msg.slice(0, 200) }, 502);
  }
});

...
async function deployAuthenticated(cwd: string, resolved: ResolvedConfig, token: string, skipBuild: boolean, jsonMode = false, noCache = false) {
  ...
  try {
    ...
    if (!jsonMode && dbDeps) {
      consola.info(
        "  ℹ Database: Creek uses the portable driver — better-sqlite3 locally, D1 remotely. Your code reads env.DATABASE in both. Do NOT rewrite for D1 manually; `creek db attach` wires the binding.",
      );
    }

...
describe("generateWorkerWrapper", () => {
  ...
  test("handles nested entry paths", () => {
    const wrapper = generateWorkerWrapper("/project/src/api/server.ts", "/project/.creek");
    expect(wrapper).toContain('from "../src/api/server"');
  });

  test("handles entry in same directory", () => {
    const wrapper = generateWorkerWrapper("/project/.creek/index.ts", "/project/.creek");
  ...
  test("includes scheduled handler with _runRequest context", () => {
    ...
    const runRequestLine = lines.findIndex(
      (l, i) => i > scheduledLine && l.includes("_runRequest(env, ctx"),
    );

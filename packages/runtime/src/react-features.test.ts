...
describe("mutate with request descriptor", () => {
  test("POST with body auto-sets Content-Type and JSON.stringify", async () => {
    ...
    const { result } = renderHook(() => useLiveQuery("/api/todos", { initialData: [] }), {
      wrapper,
    });
    ...
    const postCalls = fetchMock.mock.calls.filter(([url]: [string]) => url === "/api/items");
  ...
  test("DELETE without body sends no Content-Type", async () => {
    ...
    await act(async () => {
      await result.current.mutate({ method: "DELETE", path: "/api/items/1" }, (prev) => prev);
    });

    const delCalls = fetchMock.mock.calls.filter(([url]: [string]) => url === "/api/items/1");
...
describe("onChange callback", () => {
  test("called when data changes from refetch", async () => {
    ...
    const wrapper = createWrapper("r1", "wss://rt/test/rooms/r1/ws");
    const { result } = renderHook(() => useLiveQuery("/api/items", { onChange }), { wrapper });

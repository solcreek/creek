...
5. `creek deploy --from-github --prod --json [--project <slug>]` can
   trigger the same flow for the latest commit on demand (no push
   required). See `references/deployment-modes.md` for when to reach
   for this. Non-interactive runs still need `--prod` (or `--sandbox`).
